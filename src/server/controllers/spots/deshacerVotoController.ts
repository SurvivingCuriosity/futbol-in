...
export async function deshacerVotoController(
  ...
  const updatedSpot = await SpotService.deshacerVotoSpot(spotId, vote, userDb.id);

  await UserService.decrementUserStat(userDb.id, "votedFutbolines");

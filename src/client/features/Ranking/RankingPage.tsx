...
export default async function RankingPage() {
  ...
  const usuariosPorPuntuacion = usersParaRanking
    .sort((a, b) => b.puntuacion - a.puntuacion)
    .map((user, index) => ({ ...user, posicion: index + 1 }));
  ...
  return (
    <div className="w-full">
      ...
      <span className="hidden md:block">
        <TablaRankingUsuarios
          users={usuariosPorPuntuacion.slice(3, usuariosPorPuntuacion.length)}
        />
      </span>

      <span className="block md:hidden">
        <TablaRankingUsuariosMovil
          users={usuariosPorPuntuacion.slice(3, usuariosPorPuntuacion.length)}
        />
      </span>
    </div>

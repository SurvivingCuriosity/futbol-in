  ...
  return (
    <div className="flex flex-col md:grid grid-cols-2 gap-4 w-full">
      {items.map((item, index) => (
        <Link
          className={`w-full border p-4 rounded-2xl ${backgroundMap[item.label]} ${borderColorMap[item.label]} min-h-40 relative overflow-hidden`}
          href={item.href}
          key={`${item.label}-${index}`}
        >
          <FontAwesomeIcon
            icon={item.icon}
            className={`absolute -bottom-1 -right-1 -rotate-12 text-8xl ${iconColorMap[item.label]}`}
          />
          <div className={`flex items-center gap-2 text-4xl ${textColorMap[item.label]}`}>
